...
productCards.forEach(card => {
    ...
    card.addEventListener('click', (e) => {
        ...
        const preciosTallaJson = card.getAttribute('data-precios-talla');
        if (preciosTallaJson) {
            currentProductPrices = JSON.parse(preciosTallaJson);
        } else {
            currentProductPrices = null;
        }

        const activeSizeBtn = document.querySelector('.size-btn.active');
        let initialSizePrice = price;
        if (currentProductPrices && activeSizeBtn) {
            const s = activeSizeBtn.textContent.trim();
            if (currentProductPrices[s]) {
                initialSizePrice = currentProductPrices[s];
            }
        }

        const priceRegular = card.getAttribute('data-price-regular');
        modalPrice.textContent = `S/ ${initialSizePrice.toFixed(2)}`;
        ...
        if (colorOptionsContainer) {
            ...
            colors.forEach((color, idx) => {
                ...
                const colorName = colorNamesMap[color.toLowerCase()] || color;
...
sizeBtns.forEach(btn => {
    btn.addEventListener('click', () => {
        ...
        if (currentProductPrices) {
            ...
            if (currentProductPrices[s]) {
                modalPrice.textContent = `S/ ${parseFloat(currentProductPrices[s]).toFixed(2)}`;
            }
        }
    });
});
